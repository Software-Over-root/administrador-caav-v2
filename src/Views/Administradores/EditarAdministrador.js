...
const EditarAdministrador = () => {
    ...
    const obtenerAdmin = async (id) => {
        let res = await adminsHelper.obtenerUnAdmin(id);
        if (res.success) {
            setAdministrador(res.data);
            setFlag(true);
        } else {
            Swal.fire(
                'Error!',
                'No se logro obtener el administrador, codigo: ' + res.code,
                'error'
            )
        }
    }
    ...
    const actualizar = e => {
        e.preventDefault();
        adminsHelper.editarAdmin(administrador, administrador._id);
    }
